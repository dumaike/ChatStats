...
def google_messages_to_generic(file_path):
    with open(file_path, mode='r', encoding=Config.google_messages_encoding) as messages_file:
        data = json.load(messages_file)

    chat_record = GenericConversation()

    for message_key in data:
        if 'Message' in data[message_key] and 'SenderId' in data[message_key] and 'Timestamp' in data[message_key]:
            ...
            chat_record.message_list.append(new_chat_message)
